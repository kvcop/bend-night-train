...
Всё ниже проверено запуском на этой машине: `bend 2.0.5`, clang 18.1.3,
32 ядра, **без GPU и CUDA**. Запуск только через обёртку
...
### На этой машине (нет CUDA/GPU, clang 18)

* `-o` с `!` **собирается** (вопреки требованию «clang 19+ с `!`» из гайда) —
  на clang 18.1.3, exit 0.
* Файл `.gpu` **не создаётся** (ни для `bench_sum_gpu`, ни для ray tracer,
  ни для `pure_par_sort`). CUDA-тулчейна нет, GPU-программа не собирается.
* Без `!` и с `!` программы ведут себя одинаково и одинаково параллелятся на
  CPU. Разница в C-файле: `#define BANGS 0` против `1` и один флаг в `FID_FLAG_T`.
* `!` на JS-бэкенде игнорируется: время то же, что без `!`.
...
* `./bin --gpu 4GB`:
  * на бинарнике **без** `!` — флаг принимается и игнорируется, программа
    работает;
  * на бинарнике **с** `!` — `bend: --gpu on, but this binary found no GPU device`,
    exit 1.
* Сборка `bench_sum_gpu.bend -o ...` заняла 0.48 с (без CUDA), `bench_sum` — 1.05 с.
...
| `tools/bend tiny.bend --publish` | — | публикация в хаб, exit 0 |
...
| `bench/bench_sum_gpu.bend` (`!`) | 0.48 с | 1.10 МБ, `.gpu` нет |
| `demos/app_ray_tracer_3d/main.bend` | 1.45 с | 1.16 МБ, `.gpu` нет |
| `demos/pure_par_sort/main.bend` | 3.19 с | `sort`, `.gpu` нет |
...
* `-o` с `!` на clang 18 **работает** (гайд требует 19+). `.gpu` не появляется,
  т.к. CUDA не найдена; `--gpu` на таком бинарнике падает с
...
18. **`--gpu` на бинарнике с `!` и без GPU** — ошибка; на бинарнике без `!` —
    тихо игнорируется. Поведение зависит не только от флага, но и от наличия
    `!` в коде.
19. `Map.get`/`Map.has` — не IO, хотя возвращают пару; `<-` для них даёт
    длинную ошибку про `@-R:Type -> ...`, а не «это не IO».
20. Публикация `--publish` работает и без спроса уходит в публичный хаб — в
    скриптах это надо учитывать.
